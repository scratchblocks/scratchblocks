module.exports = function() {
  ...
  Script.prototype.draw = function(inside) {
    ...
    for (var i=0; i<this.blocks.length; i++) {
      ...
      var x = inside ? 0 : 2;
      children.push(SVG.move(x, y, block.draw()));
      this.width = Math.max(this.width, block.width);

      var diff = block.diff;
      if (diff) {
        var className = diff === '+' ? 'sb-diff-ins' : diff === '-' ? 'sb-diff-del' : '';
        var ellipse = SVG.ellipse(block.width + 4, block.height - 2, {
          class: 'sb-diff ' + className,
        })
        children.push(SVG.move(x, y + 1, ellipse))
        this.width = Math.max(this.width, block.width + 4);
      }
