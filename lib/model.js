module.exports = function() {
  ...
  Block.prototype.stringify = function(extras) {
    ...
    if ((this.info.shape === 'reporter' && this.isReporter)
  ...
  Glow.prototype.drawSelf = function() {
    ...
    if (c.isScript) {
      if (!c.isEmpty && c.blocks[0].isHat) {
        el = SVG.hatRect(this.width, this.height);
      } else if (c.isFinal) {
        el = SVG.capRect(this.width, this.height);
      } else {
        el = SVG.stackRect(this.width, this.height);
      }
    } else {
      var el = c.drawSelf(this.width, this.height, []);
    }
