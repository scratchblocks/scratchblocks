...
var scratchblocks = function () {
  ...
  function parseSpec(spec) {
    ...
    return {
      spec: spec,
      parts: parts,
      parts.filter(function(p) { return inputPat.test(p); }),
    };
  }

  var blocksBySelector = {};
  var blocks = scratchCommands.map(function(command) {
    assert(command.length <= 4); // TODO
    var block = extend(parseSpec(command[0], {
    ...
    return blocksBySelector[block.selector] = block;
  });
